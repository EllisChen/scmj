...
cc.Class({
    ...
    onGameOver_XZDD(data){
        console.log('-----------------------> onGameOver_XZDD data is ', data);
        if(data.length == 0){
...
console.log('-----------------------> onGameOver_XZDD cc.vv.gameNetMgr.conf.numberPlayers is ', cc.vv.gameNetMgr.conf.numberPlayers);
        if (numberPlayers === 3) {
            ...
            var s3Node = listRoot.getChildByName('s3');
            if (s3Node) {
                s3Node.active = false;
            }
        }
  console.log('-----------------------> onGameOver_XZDD this._gameresult ', this._gameresult); 
  console.log('-----------------------> onGameOver_XZDD this._gameover ', this._gameover); 
        ...
        for(var i = 0; i < numberPlayers; ++i){
            ...
            //Added by Ellis
            var angangsLength = 0;
            if (userData.angangs != null) {
                angangsLength = userData.angangs.length;
            }
            var wangangsLength = 0;
            if (userData.wangangs != null) {
                wangangsLength = userData.wangangs.length;
            }

            var diangangsLength = 0;
            if (userData.diangangs != null) {
                diangangsLength = userData.diangangs.length;
            }

            var numOfGangs = angangsLength + wangangsLength + diangangsLength;
    ...
    onGameOver_XLCH:function(data){
        ...
        console.log('-----------------------> onGameOver_XLCH this._gameresult ', this._gameresult); 
        console.log('-----------------------> onGameOver_XLCH this._gameover ', this._gameover); 
